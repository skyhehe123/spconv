...
class IndiceMaxPool(pccm.Class):
        ...
        for (int i : tv::KernelLoopY<int>(num_indices, block_idx_y, OneDim ? num_blocks_y : gridDim.y)) {{
            ...
            for (int j : tv::KernelLoopX<int>(num_features, block_idx_x, OneDim ? num_blocks_x : gridDim.x)) {{
                ...
                for (int k = 0; k < RS; ++k){{
                    ...
                    w = weights[k * num_features + j];
                    in += in_temp * w;
                    indices_ptr += num_indices;
                }}
                out_ptr[j] = count > 0 ? in : T(0);
            }}
        }}
        """)
        return code
    ...
    def backward_avgpool_implicit_gemm_kernel(self):
        ...
        for (int i : tv::KernelLoopY<int>(num_indices, block_idx_y, OneDim ? num_blocks_y : gridDim.y)) {{
            ...
            for (int j : tv::KernelLoopX<int>(num_features, block_idx_x, OneDim ? num_blocks_x : gridDim.x)) {{
                ...
                for (int k = 0; k < RS; ++k){{
                    out_idx = indices_ptr[0];
                    T grad_output = out_idx != -1 ? dout_features[out_idx * num_features + j] : T(0);
                    T input_val = in_ptr[j];
                    T weight = weights[k * num_features + j];
                    grad_input += grad_output * weight;
                    bool valid = out_idx != -1;
                    if (valid){{
                        atomicAdd(&dw_features[k * num_features + j], grad_output * input_val);
                    }}
                    indices_ptr += num_indices;
                }}
                din_ptr[j] = grad_input;
            }}
        }}
        """)
        return code
    ...
    @pccm.cuda.cuda_global_function
    def forward_sumpool_implicit_gemm_kernel(self):
        code = pccm.FunctionCode()
        code.targ("T")
        code.nontype_targ("OneDim", "bool", "false")
    ...
    @pccm.cuda.cuda_global_function
    def backward_sumpool_implicit_gemm_kernel(self):
        code = pccm.FunctionCode()
        code.targ("T")
        code.nontype_targ("OneDim", "bool", "false")
    ...
    def forward_avgpool_implicit_gemm(self):
        ...
        tv::dispatch<float, double, tv::half_t, tv::bfloat16_t, int8_t>(out.dtype(), [&](auto I){{
            ...
            tv::dispatch_int<0, 1>(int(num_blocks_Y > kMaxGridYZDim), [&](auto I2){{
                ...
                ws.data_ptr<const T>(), inds.data_ptr<const int>(), count_out.data_ptr<int>(), 
                out.dim(1), inds.dim(0), inds.dim(1), num_blocks_X, num_blocks_Y);
    ...
    def backward_avgpool_implicit_gemm(self):
        ...
        auto cudastream = reinterpret_cast<cudaStream_t>(stream);
        tv::dispatch<float, double>(dout.dtype(), [&](auto I){{
            using T = TV_DECLTYPE(I);
            ...
            tv::dispatch_int<0, 1>(int(num_blocks_Y > kMaxGridYZDim), [&](auto I2){{
                ...
                launcher(backward_avgpool_implicit_gemm_kernel<T, OneDim>, 
                    dout.data_ptr<const T>(), in.data_ptr<T>(), ws.data_ptr<T>(), din.data_ptr<T>(), dw.data_ptr<T>(),
    ...
    @pccm.cuda.static_function
    def forward_sumpool_implicit_gemm(self):
        code = pccm.FunctionCode()
        code.arg("out", "tv::Tensor")
        code.arg("in", "tv::Tensor")
        code.arg("inds", "tv::Tensor")
        ...
        tv::dispatch<float, double, tv::half_t, tv::bfloat16_t, int8_t>(out.dtype(), [&](auto I){{
            ...
            tv::dispatch_int<0, 1>(int(num_blocks_Y > kMaxGridYZDim), [&](auto I2){{
                constexpr bool OneDim = TV_DECLTYPE(I2)::value == 1;
                launcher(forward_sumpool_implicit_gemm_kernel<T, OneDim>, out.data_ptr<T>(), 
                in.data_ptr<const T>(), inds.data_ptr<const int>(), out.dim(1), 
                inds.dim(0), inds.dim(1), num_blocks_X, num_blocks_Y);

            }});
            TV_CHECK_CUDA_ERR_V2("sum pool fwd failed!!!");
    ...
    @pccm.cuda.static_function
    def backward_sumpool_implicit_gemm(self):
        code = pccm.FunctionCode()
        code.arg("dout", "tv::Tensor")
        code.arg("din", "tv::Tensor")
        code.arg("inds", "tv::Tensor")
        ...
        tv::dispatch<float, double, tv::half_t, tv::bfloat16_t>(dout.dtype(), [&](auto I){{
            ...
            tv::dispatch_int<0, 1>(int(num_blocks_Y > kMaxGridYZDim), [&](auto I2){{
                ...
                launcher(backward_sumpool_implicit_gemm_kernel<T, OneDim>, 
                    dout.data_ptr<const T>(), din.data_ptr<T>(), inds.data_ptr<const int>(), 
                    dout.dim(1), inds.dim(0), inds.dim(1), num_blocks_X, num_blocks_Y);
            }});
            TV_CHECK_CUDA_ERR_V2("sum pool bwd failed!!!");
