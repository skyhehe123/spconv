...
class SparseAvgPoolImplicitGemmFunction(Function):
    ...
    def forward(ctx, features: torch.Tensor, weights:torch.Tensor, 
                ...
                num_activate_out: int, calc_count):
        out, count = ops.indice_avgpool_implicit_gemm(features, weights,
                                                      indice_pairs_fwd,
                                                      num_activate_out,
                                                      calc_count)
        ctx.save_for_backward(indice_pairs_bwd, features, weights, out, count)
        return out
    ...
    @_TORCH_CUSTOM_BWD
    def backward(ctx, grad_output):
        indice_pairs_bwd, features, weights, out, count = ctx.saved_tensors
        input_bp, weight_bp = ops.indice_avgpool_implicit_gemm_backward(
            grad_output, features, weights, indice_pairs_bwd, count)
        return input_bp, weight_bp, None, None, None, None


class SparseSumPoolImplicitGemmFunction(Function):
    @staticmethod
    @_TORCH_CUSTOM_FWD
    def forward(ctx, features: torch.Tensor, 
                ...
                num_activate_out: int):
        out = ops.indice_sumpool_implicit_gemm(
            features, indice_pairs_fwd, num_activate_out)
        ctx.save_for_backward(indice_pairs_bwd)
        return out
    ...
    @_TORCH_CUSTOM_BWD
    def backward(ctx, grad_output):
        indice_pairs_bwd = ctx.saved_tensors[0]
        input_bp = ops.indice_sumpool_implicit_gemm_backward(
            grad_output, indice_pairs_bwd)
        return input_bp, None, None, None
...
indice_sumpool_implicit_gemm = SparseSumPoolImplicitGemmFunction.apply
