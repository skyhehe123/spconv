...
def indice_avgpool_implicit_gemm(features: torch.Tensor, weights: torch.Tensor,
                                 indice_pairs: torch.Tensor, num_activate_out,
                                 calc_count: bool):
    # torch.cuda.synchronize()
    # t = time.time()
    ...
    out_features_tv = torch_tensor_to_tv(out_features)
    features_tv = torch_tensor_to_tv(features)
    weights_tv = torch_tensor_to_tv(weights)
    indice_pairs_tv = torch_tensor_to_tv(indice_pairs)
    ...
    SpconvOps.avgpool_implicit_gemm_forward(out_features_tv, features_tv, weights_tv,
...
def indice_avgpool_implicit_gemm_backward(out_bp, features, weights, indice_pairs, count_out):
    # torch.cuda.synchronize()
    # t = time.time()
    ...
    dw = torch.zeros_like(weights, dtype=out_bp.dtype,
                      device=out_bp.device)
    ...
    features_tv = torch_tensor_to_tv(features)
    weights_tv = torch_tensor_to_tv(weights)
    dw_tv = torch_tensor_to_tv(dw)
    din_tv = torch_tensor_to_tv(din)
    indice_pairs_tv = torch_tensor_to_tv(indice_pairs)
    SpconvOps.avgpool_implicit_gemm_backward(out_bp_tv, features_tv, weights_tv, din_tv, dw_tv,
                                             indice_pairs_tv, count_out_tv,
                                             stream)
    return din, dw


def indice_sumpool_implicit_gemm(features: torch.Tensor, indice_pairs: torch.Tensor, num_activate_out):
    ...
    SpconvOps.sumpool_implicit_gemm_forward(out_features_tv, features_tv,indice_pairs_tv, stream)

    return out_features


def indice_sumpool_implicit_gemm_backward(out_bp, indice_pairs):
    ...
    din_tv = torch_tensor_to_tv(din)
    indice_pairs_tv = torch_tensor_to_tv(indice_pairs)
    SpconvOps.sumpool_implicit_gemm_backward(out_bp_tv, din_tv, indice_pairs_tv, stream)
    return din
