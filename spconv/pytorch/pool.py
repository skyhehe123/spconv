...
class SparseDwConv(SparseModule):
    def __init__(self,
        ...
        self.weight = Parameter( torch.zeros(*self.kernel_size, dim))
    ...
    def forward(self, input):
        ...
        with profile_ctx:
            ...
            out_features = Fsp.indice_avgpool_implicit_gemm(
                features, self.weight, pair_fwd, pair_bwd, outids.shape[0], self.training)
...
class SparseSumPool(SparseModule):
    def __init__(self,
                 ndim,
                 kernel_size: Union[int, List[int], Tuple[int, ...]] = 3,
                 stride: Optional[Union[int, List[int], Tuple[int, ...]]] = 1,
                 ...
                 record_voxel_count: bool = False,
                 name=None):
        super(SparseSumPool, self).__init__(name=name)
        self.ndim = ndim
        self.kernel_size = expand_nd(ndim, kernel_size)
        if stride is None:
            self.stride = self.kernel_size.copy()
        ...
        with profile_ctx:
            if self.indice_key is not None and datas is not None:
                ...
                if self.indice_key is not None:
                    ...
                    assert self.indice_key not in indice_dict, msg
                    indice_dict[self.indice_key] = indice_data
            out_features = Fsp.indice_sumpool_implicit_gemm(
                features, pair_fwd, pair_bwd, outids.shape[0])
...
class SparseDwConv1d(SparseDwConv):
...
class SparseDwConv2d(SparseDwConv):
    """avg pool that use real point count instead of kernel size.
    """
    def __init__(self,
                 dim,
...
class SparseDwConv3d(SparseDwConv):
    """avg pool that use real point count instead of kernel size.
    """
    def __init__(self,
                 dim,
...
class SparseSumPool1d(SparseSumPool):
    def __init__(self,
                 kernel_size,
                 stride=None,
                 padding=0,
...
class SparseSumPool2d(SparseSumPool):
    def __init__(self,
                 kernel_size,
                 stride=None,
                 padding=0,
...
class SparseSumPool3d(SparseSumPool):
    def __init__(self,
                 kernel_size,
                 stride=None,
                 padding=0,
        ...
        super(SparseSumPool3d,
              self).__init__(3,
                             ...
                             name=name)
ALL_POOL_LAYERS = set([
    SparseSumPool1d, SparseSumPool2d, SparseSumPool3d, 
    SparseDwConv3d, SparseDwConv2d, SparseDwConv1d, 
    SparseMaxPool1d, SparseMaxPool2d, SparseMaxPool3d, SparseMaxPool4d, 
    SparseDwConv, SparseMaxPool, SparseSumPool
])